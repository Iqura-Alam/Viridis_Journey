...
public class CoalEnergySource extends EnergySource {
    private static final Random rng = new Random();  // Same RNG instance for the session

    public CoalEnergySource(String id, int cost, int baseOutput, int range, int pollutionPerUnit, 
            double reliability, double minVar, double maxVar, int maintenanceCost,Tile tile) {
super(id, EnergySourceType.COAL, cost, baseOutput, range, pollutionPerUnit, reliability, minVar, maxVar, maintenanceCost,tile);
}

    @Override
    public int calculateOutput(Weather weather) {
        double weatherModifier = 1.0;  // Coal is not weather-dependent
